...
export class Game {
  ...
  constructor(container: HTMLElement, hintEl: HTMLElement, statusEl: HTMLElement) {
  ...
  private buildTargets() {
    const positions = [
      ...
      new THREE.Vector3(2.2, 0, 24),
      new THREE.Vector3(0, 0, 30),
      new THREE.Vector3(-1.8, 0, 36),
      new THREE.Vector3(2.8, 0, 42),
    ]
    ...
    for (const pos of positions) {
      ...
      const mesh = new THREE.Mesh(new THREE.CapsuleGeometry(0.35, 0.8, 6, 12), mat)
      mesh.position.copy(pos)
      mesh.position.y = 1.1
      mesh.castShadow = true
      mesh.userData.isTarget = true
      this.scene.add(mesh)
      this.targets.push({ mesh, hp: 2, alive: true })
    }
  }
  ...
  private tryFire() {
    if (!this.pointerLocked || this.fireCooldown > 0) return
    ...
    const aliveMeshes = this.targets.filter((t) => t.alive).map((t) => t.mesh)
    const hits = this.raycaster.intersectObjects(aliveMeshes, false)

    let end = origin.clone().addScaledVector(dir, MAX_RANGE)
    if (hits.length > 0) {
      const hit = hits[0]
      end = hit.point.clone()
  ...
  private updateStatus() {
    ...
    this.statusEl.textContent =
      left === 0
        ? `Kills ${this.kills} · Street clear`
        : `Kills ${this.kills} · Targets left ${left}`
  }
  ...
  private update(dt: number) {
    ...
    for (let i = this.tracers.length - 1; i >= 0; i--) {
      ...
      mat.opacity = Math.max(0, 1 - t.age * 6)
      if (t.age > 0.18) {
